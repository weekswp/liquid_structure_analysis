...
def create_motif(cluster_path: Path, label_path: Path, results_path: Path, output_data_path: Path):
    ...
    for aligned in aligned_data:
        if aligned.swapped is False:
            ...
            print("Model length: "+str(len(model)))
            print("CN + 1: "+str(cn + 1))
            print(target)
            print(model)
    ...
    atom_labels = atom_labels[~np.isnan(atom_labels)]
    ...
    centers = []
    for atom_index in range(0, (cn)):  # (cn+1) - 1 is the center atom
        _centers = []
