...
def find_map(model, target, k=3, n_choosek_flag=True, weights=None):
    ...
    print("About to call point match")
    libm.point_match(model, model.shape[0], target, target.shape[0], weights, k, output_map, n_choosek_flag)
    print("Point match called successfully")
    return output_map - 1
